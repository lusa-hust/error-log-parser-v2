...
def get_time_ats(string_time):
    ...
    string_time = string_time[:4] + " " + string_time[4:6] + " " + string_time[6:8] + " " + string_time[9:11] + " " +  string_time[12:14] + " " + string_time[15:17]
    return parse.datetime.strptime(string_time, "%Y %m %d %H %M %S")
...
def detect_log_type(line):
    ...
    for TYPE in LOG_TYPES:
        ...
        if TYPE == 'HAPROXY' or TYPE == 'HAPROXY2':
            error_info = parse.parse(log_format, line, dict(get_time_haproxy = get_time_haproxy))
        elif TYPE == 'ATS':
            error_info = parse.parse(log_format, line, dict(get_time_ats = get_time_ats))
        else:
